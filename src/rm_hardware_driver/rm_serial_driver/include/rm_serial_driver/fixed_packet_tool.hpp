// Copyright (C) 1621 RoboMaster-OSS
//
...
//
// Additional modifications and features by Chengfu Zou, 1623.
//
...
public:
  ...
  : transporter_(transporter) {
    ...
    FYT_REGISTER_LOGGER("serial_driver", "~/fyt1624-log", INFO);
  }
...
bool FixedPacketTool<capacity>::checkPacket(uint8_t *buffer, int recv_len) {
  ...
  if (recv_len != capacity) {
    FYT_INFO("serial_driver", "len is false");
    return false;
  }
  // 检查帧头，帧尾,
