...
//   if (capture_thread_.joinable()) {//hik
...
//   std::vector<rclcpp::Parameter> parameters) {//hik
...
{
  FYT_REGISTER_LOGGER("HikCameraNode", "/tmp/hik_camera.log", INFO);
  FYT_INFO("HikCameraNode", "Starting HikCameraNode!");

  MV_CC_DEVICE_INFO_LIST device_list;
  nRet_ = MV_CC_EnumDevices(MV_USB_DEVICE, &device_list);
  FYT_INFO("HikCameraNode", "Found camera count = {}", device_list.nDeviceNum);

  while (device_list.nDeviceNum == 0 && rclcpp::ok()) {
    FYT_ERROR("HikCameraNode", "No camera found!");
    FYT_INFO("HikCameraNode", "Enum state: {}", nRet_);
  ...
  if (camera_info_manager_->validateURL(camera_info_url)) {
    ...
    FYT_WARN("HikCameraNode", "Invalid camera info URL: {}", camera_info_url.c_str());
  }
  ...
  capture_thread_ = std::thread{[this]() -> void {
    ...
    FYT_INFO("HikCameraNode", "Publishing image!");
    ...
    while (rclcpp::ok()) {
      ...
      if (MV_OK == nRet_) {
        ...
        FYT_WARN("HikCameraNode", "Get buffer failed! nRet_: {}", nRet_);
        MV_CC_StopGrabbing(camera_handle_);
        MV_CC_StartGrabbing(camera_handle_);
        fail_count_++;
      }
      ...
      if (fail_count_ > 5) {
        FYT_FATAL("HikCameraNode", "Camera failed!");
        rclcpp::shutdown();
      }
    }
  }};
...
HikCameraNode::~HikCameraNode() {
  ...
  FYT_INFO("HikCameraNode", "HikCameraNode destroyed!");
}
...
void HikCameraNode::declareParameters() {
  ...
  double exposure_time = this->declare_parameter("exposure_time", 5000, param_desc);
  MV_CC_SetFloatValue(camera_handle_, "ExposureTime", exposure_time);
  FYT_INFO("HikCameraNode", "Exposure time: {}", exposure_time);
  ...
  double gain = this->declare_parameter("gain", f_value.fCurValue, param_desc);
  MV_CC_SetFloatValue(camera_handle_, "Gain", gain);
  FYT_INFO("HikCameraNode", "Gain: {}", gain);
}


rcl_interfaces::msg::SetParametersResult HikCameraNode::onSetParameters(
